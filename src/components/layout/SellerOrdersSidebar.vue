      ...
      <nav class="px-1.5 sm:px-2 pb-4 space-y-1 overflow-hidden">
        <RouterLink
          ...
          >
            ...
            <svg v-else-if="i.iconType === 'star'" xmlns="http://www.w3.org/2000/svg" class="h-5 w-5" fill="currentColor" viewBox="0 0 20 20">
...
const norm = computed(() => {
  ...
  return {
    ...
    reviews:   pick('reviews',   ['ratings', 'reviewCount'])     // ✅ NEW
  }
})

const items = computed(() => ([
  { label: 'To Ship',        to: { name: 'SellerOrdersToShip' },        count: norm.value.toShip,     iconType: 'package'  },
  { label: 'Shipping',       to: { name: 'SellerOrdersShipping' },      count: norm.value.shipping,   iconType: 'truck'    },
  { label: 'Completed',      to: { name: 'SellerOrdersCompleted' },     count: norm.value.completed,  iconType: 'check'    },
  { label: 'Cancellation',   to: { name: 'SellerOrdersCancellation' },  count: norm.value.cancelled,  iconType: 'cancel'   },
  { label: 'Return/Refund',  to: { name: 'SellerOrdersReturnRefund' },  count: norm.value.rr,         iconType: 'refresh'  },
  { label: 'Reviews',        to: { name: 'SellerOrdersReviews' },       count: norm.value.reviews,    iconType: 'star'     },  // ✅ NEW
  { label: 'Inventory',      to: { name: 'SellerOrdersInventory' },     count: norm.value.inventory,  iconType: 'inventory'}
])
)
</script>