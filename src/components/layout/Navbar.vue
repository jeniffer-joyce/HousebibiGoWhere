...
<template>
  <header
    class="sticky top-0 z-50 flex items-center justify-between border-b border-slate-200/80 bg-background-light/80 px-4 py-3 backdrop-blur-sm dark:border-slate-800/80 dark:bg-background-dark/80 sm:px-6 lg:px-8 transition-all duration-300 ease-in-out"
    :class="{ '-translate-y-full opacity-0': !showTopNav, 'opacity-100': showTopNav }">
    ...
    <div class="flex items-center gap-6">
      <RouterLink to="/" class="flex items-center gap-3 focus:outline-none focus:ring-2 focus:ring-primary rounded-lg">
        ...
        <span
          class="hidden sm:inline text-lg sm:text-xl font-bold tracking-tight text-slate-900 dark:text-white leading-none">
          HousebibiGoWhere
        </span>
      </RouterLink>
      ...
      <nav class="hidden [@media(min-width:880px)]:flex items-center gap-6">
        <RouterLink to="/"
          class="text-sm font-medium text-slate-700 hover:text-primary dark:text-slate-300 dark:hover:text-primary">Home
        </RouterLink>
        ...
        <RouterLink to="/for-sellers"
          class="text-sm font-medium text-slate-700 hover:text-primary dark:text-slate-300 dark:hover:text-primary">For
          Sellers</RouterLink>
      </nav>
    </div>
    ...
    <div class="flex items-center gap-4">
      ...
      <div v-else-if="user.isLoggedIn && !user.loading" class="flex items-center gap-4">
        ...
        <div class="hidden [@media(min-width:880px)]:flex relative profile-dropdown-container">
          <button @click.stop="showProfileDropdown = !showProfileDropdown"
            class="focus:outline-none focus:ring-2 focus:ring-primary rounded-full">
            <img :src="user.avatar || '/avatar.png'" @error="$event.target.src = '/avatar.png'" alt="avatar"
          ...
          <Transition enter-active-class="transition ease-out duration-100"
            ...
            <div v-if="showProfileDropdown"
              ...
              <div class="py-1">
                ...
                <RouterLink v-if="user.role !== 'seller'" to="/buyer-dashboard/" @click="showProfileDropdown = false"
      ...
      <button @click="toggleMobileNav"
        ...
        <svg v-if="!showMobileNav" xmlns="http://www.w3.org/2000/svg" class="h-6 w-6" fill="none" viewBox="0 0 24 24"
          stroke="currentColor">
      ...
      <nav class="mx-auto max-w-7xl px-4 py-3 sm:px-6 lg:px-8 block [@media(min-width:880px)]:hidden">
        <div class="flex flex-col gap-2">
          ...
          <div v-if="!user.isLoggedIn && !user.loading" class="flex flex-col gap-2">
            ...
            <RouterLink to="/products/" @click="closeMobileNav"
              class="rounded-lg px-3 py-2 text-sm font-medium text-slate-700 hover:bg-slate-100 dark:text-slate-300 dark:hover:bg-slate-800 transition-colors">
              Products
            </RouterLink>

            <RouterLink to="/about/" @click="closeMobileNav"
              class="rounded-lg px-3 py-2 text-sm font-medium text-slate-700 hover:bg-slate-100 dark:text-slate-300 dark:hover:bg-slate-800 transition-colors">
              About Us
            </RouterLink>

            <RouterLink to="/for-sellers/" @click="closeMobileNav"
              class="rounded-lg px-3 py-2 text-sm font-medium text-slate-700 hover:bg-slate-100 dark:text-slate-300 dark:hover:bg-slate-800 transition-colors">
              For Sellers
            </RouterLink>

            <div class="mt-2 flex gap-2">
              <RouterLink to="/signup/" @click="closeMobileNav"
              ...
              <RouterLink to="/login/" @click="closeMobileNav"
