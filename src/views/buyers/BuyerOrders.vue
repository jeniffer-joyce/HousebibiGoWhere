<template>
  <div class="flex min-h-screen bg-slate-50 dark:bg-slate-900">
    ...
    <main class="flex-1 p-6 sm:p-8 transition-all duration-300" :class="isSidebarCollapsed ? 'ml-16' : 'ml-64'">
      <div class="mx-auto w-full max-w-6xl space-y-8">
        ...
        <div class="flex items-center justify-between">
          ...
          <div class="w-full max-w-md">
            <div class="relative">
              ...
              />
              <svg class="absolute left-3 top-2.5 h-5 w-5 text-slate-400" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke="currentColor">
        ...
        <div class="rounded-2xl border border-slate-200 bg-white p-2 dark:border-slate-700 dark:bg-slate-800">
          <div class="grid grid-cols-7 gap-2">
            <button
              ...
              @click="active = t.key"
              class="flex h-11 items-center justify-center gap-2 rounded-xl border transition
        ...
        <div v-if="loading" class="flex h-48 items-center justify-center text-slate-500 dark:text-slate-400">
          <svg class="mr-2 h-5 w-5 animate-spin text-blue-600" viewBox="0 0 24 24"><circle class="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" stroke-width="4"/><path class="opacity-75" fill="currentColor" d="M4 12a8 8 0 0 1 8-8v4a4 4 0 0 0-4 4H4z"/></svg>
          Loading orders…
        </div>

        <div v-else-if="visibleOrders.length === 0" class="rounded-2xl border border-dashed border-slate-300 bg-white py-20 text-center dark:border-slate-700 dark:bg-slate-800">
        ...
        <div v-else class="space-y-6">
          ...
          >
            ...
            <div class="flex items-center justify-between border-b border-slate-100 p-4 dark:border-slate-700">
              ...
              <div class="flex items-center gap-3 text-sm">
                ...
                <span
                  ...
                  :title="statusMap[statusOf(o)]?.hint || ''"
                >
            ...
            <div class="divide-y divide-slate-100 dark:divide-slate-700">
              <div
                v-for="(it, idx) in o.products || []" :key="idx"
                class="flex items-start justify-between gap-4 p-4"
              >
                <div class="flex items-start gap-3">
                  <img :src="it.img_url" class="h-16 w-16 rounded-md object-cover" alt="">
                  <div>
                ...
                <template v-else-if="statusOf(o) === 'completed'">
                  ...
                  <button
                    ...
                    class="rounded-lg border border-slate-300 px-4 py-2 text-slate-700 hover:bg-slate-50
                           dark:border-slate-600 dark:text-slate-200 dark:hover:bg-slate-700">Request Return/Refund</button>
                </template>
    ...
    <!-- Cancel confirm -->
    <div v-if="showCancelConfirm" class="fixed inset-0 z-[70] flex items-center justify-center bg-black/40" @click="showCancelConfirm=false">
      <div class="w-full max-w-md rounded-2xl bg-white p-6 shadow-xl sm:mx-4 dark:bg-slate-800" @click.stop>
        ...
        <p class="mt-1 text-sm text-slate-600 dark:text-slate-300">
          Are you sure you want to cancel order <span class="font-medium">{{ orderToCancel?.orderId }}</span>? This action can’t be undone.
        </p>
        <div class="mt-5 flex justify-end gap-2">
          <button @click="showCancelConfirm=false" class="rounded-lg border border-slate-300 px-4 py-2 text-slate-700 hover:bg-slate-50 dark:border-slate-600 dark:text-slate-200 dark:hover:bg-slate-700">No, keep order</button>
...
function orderGrand(o) {
  ...
  const sum = (o.products || []).reduce((acc, p) => acc + (p.totalPrice ?? (p.price * (p.quantity ?? 1))), 0)
