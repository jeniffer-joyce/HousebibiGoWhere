...
<template>
  <section class="space-y-4">
    ...
    <header class="flex items-center justify-between">
      <div>
        <h2 class="text-2xl font-bold text-slate-900 dark:text-white">Completed Orders</h2>
    ...
    >
        <div class="w-full max-w-5xl overflow-hidden rounded-2xl bg-white shadow-2xl ring-1 ring-slate-200 dark:bg-slate-900 dark:ring-slate-800">
        <!-- Header -->
        <div class="flex items-start justify-between border-b border-slate-200 px-5 py-4 dark:border-slate-800">
            ...
            <button
                class="rounded-md border px-3 py-1.5 text-sm hover:bg-slate-50 dark:border-slate-600 dark:text-slate-200 dark:hover:bg-slate-800/60"
            ...
            <button class="rounded-md bg-blue-600 px-3 py-1.5 text-sm text-white" @click="closeDetails">
                Back
            </button>
            </div>
        </div>
        ...
        <div class="grid grid-cols-1 gap-4 p-5 lg:grid-cols-3">
            ...
            <div class="rounded-xl border p-4 dark:border-slate-800">
                ...
                <div class="mt-1"><span class="font-medium">Channel:</span>
            ...
            <div class="overflow-hidden rounded-xl border dark:border-slate-800">
                ...
                <table class="w-full border-collapse text-sm">
                    ...
                    <tr>
                        <th class="px-3 py-2 text-left  font-semibold text-slate-700 dark:text-slate-200">Product</th>
                        <th class="px-3 py-2 text-left  font-semibold text-slate-700 dark:text-slate-200">Variant</th>
                        <th class="px-3 py-2 text-center font-semibold text-slate-700 dark:text-slate-200">Qty</th>
                        <th class="px-3 py-2 text-right font-semibold text-slate-700 dark:text-slate-200">Unit</th>
                        <th class="px-3 py-2 text-right font-semibold text-slate-700 dark:text-slate-200">Total</th>
                    </tr>
                    </thead>
                    <tbody>
                    ...
                    <tr v-for="(p,i) in dProducts" :key="i" class="border-t border-slate-100 dark:border-slate-800">
                        <td class="px-3 py-2 text-slate-800 dark:text-slate-200">{{ p.item_name || p.name || '—' }}</td>
                        <td class="px-3 py-2 text-slate-700 dark:text-slate-300">{{ p.size || p.variant || '-' }}</td>
                        <td class="px-3 py-2 text-center text-slate-700 dark:text-slate-300">{{ Number(p?.quantity ?? 1) }}</td>
                        <td class="px-3 py-2 text-right  text-slate-900 dark:text-white">{{ fmtCurrency(unitPrice(p)) }}</td>
                        <td class="px-3 py-2 text-right  text-slate-900 dark:text-white">{{ fmtCurrency(lineTotal(p)) }}</td>
                    </tr>
                    </tbody>
                </table>
                </div>
    ...
    <teleport to="body">
      ...
      >
        <div class="w-full max-w-4xl overflow-hidden rounded-2xl bg-white shadow-2xl ring-1 ring-slate-200 dark:bg-slate-900 dark:ring-slate-800">
          <!-- Header -->
          <div class="flex items-center justify-between border-b border-slate-200 px-5 py-4 dark:border-slate-800">
            ...
            <div class="flex items-center gap-2">
              <button class="rounded-md border px-3 py-1.5 text-sm hover:bg-slate-50 dark:border-slate-600 dark:text-slate-200 dark:hover:bg-slate-800/60" @click="printInvoice">
                Print
              </button>
              <button class="rounded-md bg-blue-600 px-3 py-1.5 text-sm text-white" @click="closeInvoice">
                Back
              </button>
            </div>
          </div>
          ...
          <!-- Printable area -->
          <div id="invoice-print-area" class="max-h-[70vh] overflow-auto p-6">
            <div class="mx-auto max-w-3xl">
              <div class="rounded-2xl border border-slate-200 p-6 shadow-sm dark:border-slate-700">
                <div class="mb-4">
                  <h1 class="m-0 text-2xl font-semibold text-slate-900 dark:text-white">Invoice</h1>
                  <div class="text-sm text-slate-500 dark:text-slate-400">Order #{{ activeId }}</div>
                </div>
                ...
                <table class="w-full border-collapse text-sm">
                    ...
                    <tr>
                        ...
                        <th class="px-3 py-2 text-left  font-semibold text-slate-700 dark:text-slate-200">Variant</th>
                        <th class="px-3 py-2 text-center font-semibold text-slate-700 dark:text-slate-200">Qty</th>
                        <th class="px-3 py-2 text-right font-semibold text-slate-700 dark:text-slate-200">Unit Price</th>
                        <th class="px-3 py-2 text-right font-semibold text-slate-700 dark:text-slate-200">Total</th>
                    </tr>
                    </thead>
                    <tbody>
                    <tr v-if="(activeProducts || []).length === 0">
                        <!-- 5 columns now (added Variant) -->
                        <td colspan="5" class="px-3 py-3 text-center text-slate-500 dark:text-slate-400">No items</td>
                    </tr>
                    ...
                    >
                        ...
                        <!-- NEW: Variant / Size column -->
                        <td class="px-3 py-2 text-slate-700 dark:text-slate-300">
                        {{ p.size || p.variant || '-' }}
                        </td>

                        <td class="px-3 py-2 text-center text-slate-700 dark:text-slate-300">
                        {{ Number(p?.quantity ?? 1) }}
                        </td>
                        <td class="px-3 py-2 text-right text-slate-900 dark:text-white">
